...
//*****************************************************************************
#if !NO_SYS
#define ETH_THREAD_PRIO	        		9	//3: same level as ENET interrupt

#define ETH_THREAD_STACKSIZE			256
#endif
...
lwIPPrivateInit(void *pvArg)
{
    ...
    xTaskCreate(lwIPInterruptTask, (portCHAR *)"eth_int         ",
    		ETH_THREAD_STACKSIZE, 0, tskIDLE_PRIORITY + 7,
                0);
#endif
#endif
...
lwIPInit(uint32_t ui32SysClkHz, const uint8_t *pui8MAC, uint32_t ui32IPAddr,
         uint32_t ui32NetMask, uint32_t ui32GWAddr, uint32_t ui32IPMode, void *net_if)
{
    //
    ...
    if (net_if != NULL){
        g_sNetIF = (struct netif *) net_if;
    }else{
        // alocar um net_if e passar o ponteiro
    }
